...
void simple_protocol_data_frame_read(uint16_t addr,uint16_t start,uint8_t count,void* data,volatile void** addr_ptrs) {
    memcpy(&((uint8_t*)(addr_ptrs[addr]))[start],data,count);
}

void simple_protocol_id_write(simple_protocol_link_layer_t* link) { //запись фрейма

}

void simple_protocol_id_read(simple_protocol_link_layer_t* link,volatile void** addr_ptrs) { //чтение фрейма
    simple_protocol_id_rw_t read;
    memcpy(&read,link->read.frame.data,ECU_PRESENTATION_LAYER_RW_COUNT);
    simple_protocol_data_frame_read(read.addr,read.start,read.count,&link->read.frame.data[ECU_PRESENTATION_LAYER_RW_COUNT],addr_ptrs);
}
...
void simple_protocol_id_handler(simple_protocol_link_layer_t* link,volatile void** addr_ptrs) {
    switch(link->read.frame.head.id) {
    case ECU_SESSION_LAYER_ID_WRITE:  simple_protocol_id_write(link);
        break;
...
void simple_protocol_crc_handler(simple_protocol_link_layer_t* link,volatile void** addr_ptrs) {
